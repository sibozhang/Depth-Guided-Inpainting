...
double min_time = 1534313570, max_time = 1534313581;
char pose_folder[512];
const int NUM_THREADS = 1;
std::vector<bool> is_idle(NUM_THREADS, true);
int main(int argc, char **argv) {

//	cv::Mat sub_img = cv::imread("../data/pandora_liang/set2/1534313570-1534313581_results/sub_img.jpg");
//	cv::Mat templ = cv::imread("../data/pandora_liang/set2/1534313570-1534313581_results/temp_img_2c_warped.jpg");
//	cv::Mat templ_mask = cv::imread("../data/pandora_liang/set2/1534313570-1534313581_results/warped_mask.jpg");
//
//	cv::Mat result;
//	cv::matchTemplate(sub_img, templ, result, CV_TM_SQDIFF, templ_mask);
//
//	double minVal, maxVal;
//	cv::Point minLoc, maxLoc;
//	cv::minMaxLoc(result, &minVal, &maxVal, &minLoc, &maxLoc, cv::Mat());
//
//	cv::rectangle(sub_img,
//				cv::Rect(minLoc.x, minLoc.y, templ.cols, templ.rows),
//				cv::Scalar(0, 255, 0));
//	cv::imwrite("../data/pandora_liang/set2/1534313570-1534313581_results/test1_det_img.jpg",
//						sub_img);
//	exit(1);
...
//	cv::Mat sub_img_float(200, 200, CV_32FC3);
//	cv::Mat sub_dmap(200, 200, CV_32FC1);
//	int from_to[] = { 0, 2 };
//	cv::mixChannels(&sub_dmap, 1, &sub_img_float, 1, from_to, 1);
//	std::cout<<"done";
//	exit(1);
	...
	sprintf(pose_folder, "../data/pandora_liang/set2/%llu-%llu",
	...
	char mask_path[512];
	sprintf(mask_path, "../data/pandora_liang/set2/%llu-%llu_mask/",
			(long long) (min_time), (long long) (max_time));
...
//            int from_to[] = { 0, 0 };
//            cv::Mat uvc0(uv_map.size(), CV_32FC1), uvc0_img;
//            cv::mixChannels(&uv_map, 1, &uvc0, 1, from_to, 1);
//            cv::normalize(uvc0, uvc0_img, 0, 255, cv::NORM_MINMAX, CV_8U);
//            cv::imwrite("../data/pandora_liang/set2/1534313570-1534313581_results/uv.png", uvc0_img);
		...
		if (file_name.at(0) == 'b') {
			...
			if (min_time < lidar_end_time && lidar_end_time < max_time) {
				...
				auto show_img = CloudImageProjectionWithMask(cloud, R, T, K,
						...
						dilated_mask_time_to_cvmat[closest_img_time]);
...
//			cv::imwrite("/disk1/tmp/img.jpg", img_time_to_cvmat[m.first]);
			//exit(0);
		}
	}
	...
	for (auto &img : img_time_to_cvmat) {
		...
		if (time_diff < 0.05) {
            ...
            if (fs.isOpened()) {
                ...
                fs["optR"] >> optR;
                fs.release();
//                std::cout << "optimized R exists for "<< std::setprecision(20)<<img.first << std::endl;
...
//            cv::cvtColor(output_img, output_img, CV_GRAY2BGR);
//            applyColorMap(output_img, output_img, cv::COLORMAP_JET);
//            imwrite("../data/pandora_liang/set2/1534313570-1534313581_results/depthcolor.png", output_img);
//            output_img = output_img/2 + img.second/2;
//            imwrite("../data/pandora_liang/set2/1534313570-1534313581_results/depthcoloroverlay.png", output_img);
//            exit(1);
...
//            auto aimg= CloudImageProjectionZBuffer(proj_map, R, T, K, img_time_to_cvmat[img.first]);
//            imwrite("../data/pandora_liang/set2/1534313570-1534313581_results/depthcoloroverlay.png", aimg);
//            exit(1);
...
//			auto f =
//					[](std::vector<PointXYZIS> proj_map, cv::Mat const R, cv::Mat const T,
//							cv::Mat const K, cv::Mat inp_img, const cv::Mat mask, double ts, int thread_idx,
//							std::map<double, cv::Mat> img_proj_mats,
//							std::map<double, cv::Mat> img_time_to_cvmat,
//							std::map<double, cv::Mat> mask_time_to_cvmat) {
//
...
//						} else {
//							optR = RotationOptimizer(proj_map, R, T, K, inp_img,
//									mask);
...
//						}
//
////						auto show_img = CloudImageProjectionGuidedInpaintingMultiScale1(
////								proj_map, optR, T, K, inp_img, mask, inp_init, img_proj_mats, ts);
//						auto show_img = InpaintingByDenseMapBP(
//								proj_map, optR, T, K, inp_img, mask, inp_init, img_proj_mats, ts,
//								img_time_to_cvmat, mask_time_to_cvmat);
//
//						char result_folder[512];
//						sprintf(result_folder,
//								"../data/pandora_liang/set2/%llu-%llu_results",
//								(long long) (min_time), (long long) (max_time));
//
//						cv::imwrite(
//								std::string(result_folder) + "/inpainted_"
...
//
//						is_idle[thread_idx] = true;
//					};
//
//			bool no_idle_thread = true;
//			while (no_idle_thread) {
//				for (int t = 0; t < NUM_THREADS; t++) {
//					if (is_idle[t]) {
//						is_idle[t] = false;
//						if (myThreads[t].joinable())
//							myThreads[t].join();
//						myThreads[t] = std::thread(f, proj_map, R, T, K,
//								inp_img, mask, img.first, t, img_proj_mats, img_time_to_cvmat, mask_time_to_cvmat);
//						no_idle_thread = false;
//						break;
//					}
//				}
//
//			}
////			myThreads[img_idx % NUM_THREADS] = std::thread(f, proj_map, R, T, K,
////					inp_img, mask, img.first);
////			img_idx++;
////			if (img_idx % NUM_THREADS == 0) {
...
//            auto show_img = InpaintingByDenseMap(
//                    proj_map, optR, T, K, inp_img, mask, inp_init, img_proj_mats, ts,
//                    img_time_to_cvmat, mask_time_to_cvmat);
